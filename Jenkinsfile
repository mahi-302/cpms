  ...
  agent {label 'main'}
    stages{
       stage('hosting application'){
        steps{
          sh "ls"
          sh "aws rds create-db-instance --db-instance-identifier test-mysql2 --db-name cpms --db-instance-class db.t2.micro --vpc-security-group-ids sg-08625c34c68a22785 --engine mysql --engine-version 5.7 --db-parameter-group-name default.mysql5.7 --publicly-accessible  --master-username admin --master-user-password mahesh8842 --allocated-storage 10 --region us-east-1"
          sleep(300)
          script{
              def cmd = "aws rds describe-db-instances --db-instance-identifier test-mysql2 --region us-east-1"
